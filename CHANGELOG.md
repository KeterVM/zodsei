...
- **Test Expectations Alignment**: Corrected test assertions to match actual axios adapter behavior
  - Tests now correctly expect relative paths (e.g., `/users`) instead of full URLs
  - `AxiosAdapter` passes relative paths to `axios.request()`, letting axios handle baseURL combination internally
  - Prevents double-prefixing issues when baseURL is a relative path
  - Aligns with standard axios behavior where the adapter provides relative paths and axios instance manages baseURL

### Technical Details

- **`AxiosAdapter.createAxiosConfig()`**: Maintains clean separation of concerns
  - Passes relative endpoint paths directly to axios config
  - Axios instance automatically combines `defaults.baseURL` with the relative path
  - Avoids manual URL construction that could cause double-prefixing
  - Follows axios best practices for URL handling

## [1.1.0] - 2025-11-23

### Changed

- Initial release with updated adapter configuration
